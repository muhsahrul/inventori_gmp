<div class="modal fade" id="detail_validasi_pr" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" data-backdrop="static">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td><input type="text" class="form-control text-right" value="<?= $subvalue['gramasi']; ?>" readonly></td>
            ...
            <div class="modal-footer justify-content-between">
                <select class="form-control select2-min" id="select_validasi" name="select_validasi" data-placeholder="-- Pilih Validasi --">
                    <option></option>
                    <?php foreach ($status_validasi as $value) { ?>
                        <option value="<?= $value['id']; ?>" <?= $row['m_status_validasi_inv_purchasing'] == $value['id'] ? 'selected' : ''; ?>><?= $value['nama']; ?></option>
                    <?php } ?>
                </select>
...
<script>
    ...
    $('#select_validasi').change(function() {
        let nomor = "<?= $row['nomor']; ?>";
        let status = $('select[name=select_validasi] option').filter(':selected').val();
        console.log(status);
        let x = confirm("Apakah ingin validasi \"" + nomor + "\"?");
        if (x) {
            $('.overlay-wrapper').attr('hidden', false);
            let id = "<?= $row['id']; ?>";
            $.ajax({
                url: "<?= site_url(); ?>purchasing/validasiPr/updateValidasiPr/" + id,
                method: "POST",
                data: {
                    nomor: nomor,
                    status: status
