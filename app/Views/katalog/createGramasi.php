<div class="modal fade" id="create_gramasi" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" data-backdrop="static">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Tambah Gramasi</h5>
            ...
            <form action="" method="POST" id="form_create_gramasi">
                <?= csrf_field(); ?>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2-full" name="bahan" id="bahan" data-placeholder="-- Pilih Bahan --" style="width: 100%">
                                ...
                                <?php foreach ($bahan as $value) { ?>
                                    <option value="<?= $value['id']; ?>" data-bahan="<?= $value['nama']; ?>"><?= $value['alias']; ?></option>
                                <?php } ?>
                            </select>
                        </div>
                        <div class="form-group">
                            <label for="gramasi">Gramasi</label>
                            <input type="number" class="form-control" name="gramasi" autocomplete="off">
                        </div>
                    </div>
                </div>
...
<script>
    ...
    $('#btn_submit').click(function() {
        ...
        let data = $('#form_create_gramasi').serialize();
        $.ajax({
            url: "gramasi/saveGramasi",
            data: data,
            type: "POST",
            dataType: "JSON",
